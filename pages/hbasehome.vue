<template>
  <v-container>
    <h1>HBASE policies</h1>
    <router-link to="/hbase" style="text-decoration: none"
    ...
    <v-card v-for="policy in policies" :key="policy.id" flat class="mt-3 ml-10 mr-15">
      <v-layout class="pa-3" wrap :class="`${policy.status}`">
        ...
        <v-flex xs1 md1 class="pt-2" justify-end>
          <v-btn small icon :to="{ name: 'hbase', params: { policy } }"
...
export default {
  ...
  methods: {
    async del(pol) {
      ...
      await deleteDoc(doc(db, 'hbase', pol))
      this.getData()
    },
    async getData() {
      const db = getFirestore()
      const q = query(collection(db, 'hbase'))
      const temp = {}
