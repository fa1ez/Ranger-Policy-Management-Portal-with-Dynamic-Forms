...
export default {
  ...
  created() {
    axios.get('http://localhost:3034/hbase_form').then((res) => {
      this.dats = res.data
    })
  ...
  methods: {
    async store(ev, router) {
      ...
      if (this.formdata.f_id) {
        ...
        await setDoc(doc(db, 'hbase', this.formdata.f_id), temp)
      } else {
        const docRef = await addDoc(collection(db, 'hbase'), this.formdata)
        console.log('Document written with ID: ', docRef.id)
      }
      router.push('hbasehome')
    },
  },
}
</script>
