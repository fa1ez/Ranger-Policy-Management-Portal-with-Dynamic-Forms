...
export default {
  ...
  created() {
    axios.get('http://localhost:3035/presto_form').then((res) => {
      this.dats = res.data
    })
  ...
  methods: {
    async store(ev, router) {
      ...
      if (this.formdata.f_id) {
        ...
        await setDoc(doc(db, 'presto', this.formdata.f_id), temp)
      } else {
        const docRef = await addDoc(collection(db, 'presto'), this.formdata)
        console.log('Document written with ID: ', docRef.id)
      }
      router.push('prestohome')
    },
  },
}
</script>
