<template>
  <v-container>
    <h1>PRESTO policies</h1>
    <router-link to="/presto" style="text-decoration: none"
    ...
    <v-card v-for="policy in policies" :key="policy.id" flat class="mt-3 ml-10 mr-15">
      <v-layout class="pa-3" wrap :class="`${policy.status}`">
        <v-flex xs6 md3>
        ...
        <v-flex xs1 md1 class="pt-2" justify-end>
          <v-btn small icon :to="{ name: 'presto', params: { policy } }"
...
export default {
  ...
  methods: {
    async del(pol) {
      ...
      await deleteDoc(doc(db, 'presto', pol))
      this.getData()
    },
    async getData() {
      const db = getFirestore()
      const q = query(collection(db, 'presto'))
      const temp = {}
